...
| Terraform state bucket | `TFSTATE_BUCKET` repo secret, the name from bootstrap's `state_bucket_name` output | Settings → Secrets and variables → Actions → Secrets |
...
Values aren't shown here on purpose. The account id was in this table until 2026-09-18, a public repo is no place for it even though it isn't a credential, so `CI_ROLE_ARN` and `KMS_KEY_ARN` are variables (visible in Settings to anyone with repo access) and the bucket name moved to a secret (write-only after creation, GitHub never displays it again).
